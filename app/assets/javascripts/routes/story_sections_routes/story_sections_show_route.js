Seanchai.StorySectionsShowRoute = Ember.Route.extend({
  ...
  setupController: function(controller, model) {
    ...
    if (model.get('next_section')) {
      var next_section = this.get('store').find('story_section', model.get('next_section'));
    ...
    if (model.get('previous_section')) {
      var previous_section = this.get('store').find('story_section', model.get('previous_section'));
